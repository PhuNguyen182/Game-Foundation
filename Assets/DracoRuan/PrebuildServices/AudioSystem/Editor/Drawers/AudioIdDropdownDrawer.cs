...
{
    /// <summary>
    /// Shared drawing for the id and channel dropdowns.
    /// </summary>
    /// <remarks>
    ...
    /// the only evidence of what the field used to point at, in a file nobody is looking at.</para>
    ///
    /// <para>Repaint cost is one integer comparison: the arrays are rebuilt only when
    /// <see cref="AudioIdIndex.Version"/> changes, not per frame.</para>
    /// </remarks>
    public abstract class AudioIdDropdownDrawer : PropertyDrawer
    {
        ...
        {
            if (this._cachedVersion == AudioIdIndex.Version)
                return;

            this._ids = this.SourceIds;
            this._labels = this.SourceLabels;
            this._cachedVersion = AudioIdIndex.Version;
        }
        ...
        {
            if (!AudioIdIndex.TryGetOwnerPath(id, out string path))
                return;
