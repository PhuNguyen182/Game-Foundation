...
{
    /// <summary>
    /// Turns an audio id into the C# member name generated for it, and proposes an ASCII id when
    /// the author typed something the charset will not take.
    /// </summary>
    /// <remarks>
    /// <para>The mapping is deliberately almost the identity. The only adjustments made are the two
    /// that the id charset permits and C# does not: a leading digit, and a reserved keyword.
    /// Casing is never touched, because re-casing <c>button_click</c> to <c>ButtonClick</c> is the
    /// fastest way to manufacture a collision with an id that was already <c>ButtonClick</c>.</para>
    ///
    ...
    ///
    /// <para>Non-ASCII characters are written as escapes throughout, so the behaviour cannot change
    /// if a tool ever rewrites this file in another encoding.</para>
    /// </remarks>
    public static class AudioIdSanitizer
    {
        ...
        /// <summary>
        /// The member name to emit for <paramref name="audioId"/>, or a rejection explaining why
        /// nothing can be emitted for it.
        /// </summary>
        public static AudioIdSanitizeResult ToMemberName(string audioId)
        {
            if (!AudioIdFormat.IsValid(audioId, out string error))
                return AudioIdSanitizeResult.Rejected(WithSuggestion(error, audioId));

            string member = audioId.Replace('-', '_');
            ...
            if (string.Equals(member, audioId, StringComparison.Ordinal))
                return AudioIdSanitizeResult.Ok(member);

            return AudioIdSanitizeResult.Adjusted(
                member, $"'{audioId}' cannot be a C# member name as written, so it generates as '{member}'.");
        }
        ...
        public static string Suggest(string rawText)
        {
            ...
            foreach (char character in decomposed)
            {
                ...
                if (!AudioIdFormat.IsAllowedCharacter(character))
                {
        ...
        private static string WithSuggestion(string error, string audioId)
        {
            string suggestion = Suggest(audioId);

            bool worthOffering = !string.IsNullOrEmpty(suggestion)
                                 && !string.Equals(suggestion, audioId, StringComparison.Ordinal)
