...
namespace AYellowpaper.SerializedCollections.Editor
{
    public class SerializedDictionaryInstanceDrawer
    {
        ...
        {
            var list = new ReorderableList(_pagedIndices, typeof(int), true, true,
                _dictionaryAttribute == null || _dictionaryAttribute.CanAddElement,
                _dictionaryAttribute == null || _dictionaryAttribute.CanRemoveElement);
            object o = ListProperty.serializedObject.targetObject;
        ...
        private void OnDrawElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            bool isReadOnlyValue = _dictionaryAttribute == null ? false : _dictionaryAttribute.IsReadOnlyValue;
            DrawGroupedElement(valueRect, lineRightSpace, valueProperty, valueDisplayData.EffectiveDisplayType, isReadOnlyValue);
        }

        private void DrawGroupedElement(Rect rect, int spaceForProperty, SerializedProperty property, DisplayType displayType, bool isReadOnly)
        {
        ...
        private void OnAdd(ReorderableList list)
        {
            ...
            if (_dictionaryAttribute != null && !string.IsNullOrEmpty(_dictionaryAttribute.CustomAddAction))
            {
                object targetObject = ListProperty.serializedObject.targetObject;
                object dictList = _fieldInfo.FieldType.GetField(SerializedDictionaryDrawer.SerializedListName, BindingFlags.Instance | BindingFlags.NonPublic)
                    .GetValue(SCEditorUtility.GetPropertyValue(ListProperty, targetObject, 1));

                object source = SCEditorUtility.GetPropertyValue(ListProperty, targetObject, 2);
                int index = actualTargetIndex + (_activeState.ListSize == 1 ? 0 : 1);
                ((IList)dictList)[index] = source.GetType().GetMethod(_dictionaryAttribute.CustomAddAction, BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)
                    .Invoke(source, null);
                object dict = SCEditorUtility.GetPropertyValue(ListProperty, targetObject);
